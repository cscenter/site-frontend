...
onReady(async () => {
  ...
  // Global polyfills
  await Promise.all([loadFetchPolyfill(), polyfillElementMatches(), loadBootstrapNativePolyfill()]);
...
function initTopMenu() {
  ...
  let menuRightBlock =
    document.querySelector('.dropdown-user-menu') || document.querySelector('.menu-btn-reg');
...
function loadSVGSprites() {
  window.__CSC__.sprites.forEach(url => {
    ky.get(url)
      ...
      .then(svgDefs => {
        document.querySelector('.svg-inline').insertAdjacentHTML('beforeend', svgDefs);
      });
  });
}
